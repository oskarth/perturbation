...
Is there a unique positive solution? We know $f(0) = E_0 \overline{S}
> 0$

TODO: I don't understand the reasoning here, Yishao page
1. Q1. Wouldn't the reason be because the discriminant != 0? How would
we know that? Why is $E_0 \overline{S} = c_1 c_2$? See Oskar p6 and
before.
...
With this we get the following inequalities. First, the discriminant o
f(c) is positive:
\begin{equation}
  (E_0 + K_M + \overline{S})^2 - 4 E_0 + \overline{S} > 0
\end{equation}

For a quadratic equation $x^2 + bx + c$, we have the equality for its roots:
\begin{align}
  x_1 + x_2 &= \frac{-b + \sqrt{\Delta}}{2} + \frac{-b - \sqrt{\Delta}}{2}, \\
            &= - b
\end{align}

Hence we have our second inequality:
\begin{equation}
  c_1 + c_2 = E_0 + K_M + \overline{S} \\
  \iff c_2 = E_0 + K_m + \overline{S}
\end{equation}
