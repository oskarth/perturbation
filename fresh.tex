...
\iffalse
TODO: remove this?
&= \Big(C - \frac{(E_0 + K_M + \overline{S})}{2}\Big)^2 + E_0 \overline{S} -
\frac{(E_0 + K_M + \overline{S})^2}{4}
\fi
...
TODO: Is it really a matter of cases? case 1 valid for case 2
...
TODO: Why is this what we investigate? Comes from a rewrite of Pade
approx, but we don't know that. It's also not really a case 2, since
we are still dealing with c1 only.
...
\begin{align}
   c_1 &= \frac{2 E_0 \overline{S}}
              {(E_0 + K_M + \overline{S})(1 + (1 -
              O(\frac{4 E_0  \overline{S}}
                     {(E_0 + K_M + \overline{S})^2})))} \\
      &\approx \frac{E_0 \overline{S}}
                    {E_0 + K_M + \overline{S}}
\end{align}

TODO: Wording We got here without the use of Pad\'{e} approximation.
...
TODO: Missing something about Borghans here I think.
...
TODO: Why are we checking $\overline{S}=E_0$? Borghans something.

Then our approximation is possible.

TODO: Why? And which exactly? C approx above?

Next thing is on my (previous) equation (85) which is now... [0324]
1/eps one, ie 109 at the moment. That is a separate page.

I think for now, grok the above. Re-read Borghans and understand.

\subsubsection{Resume previous writings}
